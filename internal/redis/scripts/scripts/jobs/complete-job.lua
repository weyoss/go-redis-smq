-- internal/redis/scripts/scripts/jobs/complete-job.lua
--
-- Copyright (c)
-- Weyoss <weyoss@outlook.com>
-- https://github.com/weyoss
--
-- Atomically completes a background job
--
...
if statusStr == completedStatus then
    -- Job already completed, just remove from processing list and clean up worker link
...
elseif statusStr == failedStatus then
    -- Job already failed, just remove from processing list and clean up worker link
    redis.call('LREM', backgroundJobsProcessingKey, 0, jobId)
    redis.call('DEL', jobWorkerKey)
    return -1  -- Already failed

elseif statusStr == canceledStatus then
    -- Job already cancelled, just remove from processing list and clean up worker link
    redis.call('LREM', backgroundJobsProcessingKey, 0, jobId)
    redis.call('DEL', jobWorkerKey)
